...
- **#17 Catalog parse resilience** — switched from reading the whole 2 MB file into a string + full
  ...
  a ~128 MB heap). Crucially it is resilient: a malformed entry is skipped individually, and a JSON
  syntax error mid-stream keeps every entry parsed before it instead of collapsing the whole
  catalogue to empty. Added tests for partial-on-syntax-error and one that parses the actual shipped
  catalog.json (8 total, incl. real file), so a broken hand edit fails CI rather than showing an
  empty home screen on the stick.
