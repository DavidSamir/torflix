...
import com.myflix.core.player.PlaybackError
...
import com.myflix.core.ui.focus.RemoteKeys
import com.myflix.core.ui.focus.onRemoteKey
...
private fun PlayerErrorOverlay(
    ...
    ErrorState(
        ...
        primaryActionLabel = if (error is PlaybackError.Unsupported) null else "Retry",
        onPrimaryAction = if (error is PlaybackError.Unsupported) null else onRetry,
        ...
        modifier = modifier
            ...
            .background(
                Brush.verticalGradient(
                    listOf(MyflixColors.Transparent, MyflixColors.ScrimStrong),
                ),
            )
...
private fun Timeline(
    ...
    Box(
        Modifier
            .fillMaxWidth()
            .height(6.dp)
            .background(MyflixColors.ProgressTrack),
    ) {
        Box(
            Modifier
                .fillMaxWidth(bufferedFraction)
                .fillMaxHeight()
                .background(MyflixColors.SurfaceHighest),
        )
        Box(
            Modifier
                .fillMaxWidth(fraction)
                .fillMaxHeight()
                .background(if (isScrubbing) MyflixColors.Focus else MyflixColors.ProgressFill),
        )
    }
}
