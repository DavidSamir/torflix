...
import com.torfilx.core.data.database.SyncMarkerEntity
...
class MediaRepository @Inject constructor(
    ...
    suspend fun refreshLibrary(full: Boolean = false): Int = withContext(ioDispatcher) {
        ...
        while (true) {
            ...
            if (page > MAX_PAGES) {
                TorfilxLog.w(TAG, "Library refresh stopped at $MAX_PAGES pages")
                break
            }
        }
        ...
        if (response.item.type == MediaType.SHOW) {
            ...
            seasons.forEach { season ->
                val episodes = runCatching { remote.episodes(id, season.number, season.id) }
                    .onFailure { TorfilxLog.w(TAG, "Episodes failed for ${season.id}", it) }
    ...
    suspend fun search(query: String): List<SearchResult> = withContext(ioDispatcher) {
        ...
        TorfilxLog.w(TAG, "Remote search failed, using local cache", remoteResults.exceptionOrNull())
